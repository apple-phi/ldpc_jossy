...
res = []
with open(ROOT_DIR / "data" / "results.csv", "r") as f:
    rr = csv.reader(f)
    for row in rr:
        res.append(row)
...
for mystd in [11, 16]:
    ...
    if mystd == 11:
        Rrange = [0.5, 0.667, 0.75, 0.833]
    else:
        Rrange = [0.667, 0.75, 0.5, 0.833]
    ...
    for myR in Rrange:
        for mytt in [0, 1]:
            if (
                np.nonzero(
                    np.logical_and(
                        np.logical_and(std == mystd, np.abs(R - myR) < 0.01), mytt == tt
                    )
                )
            )[0].size == 0:
                continue
            ...
            if mytt == 0:
                plt.title(
                    "802.%(st)d, R=%(rt).3g, Z=27,54,81" % {"st": mystd, "rt": myR}
                )
            else:
                plt.title(
                    "802.%(st)d, R=%(rt).3g, Type B, Z=27,54,81"
                    % {"st": mystd, "rt": myR}
                )
            plt.yscale("log")
            ...
            for myZ in [27, 54, 81]:
                indx = np.nonzero(
                    np.logical_and(
                        np.logical_and(
                            np.logical_and(std == mystd, np.abs(R - myR) < 0.01),
                            tt == mytt,
                        ),
                        Z == myZ,
                    )
...
for myZ in [27, 54, 81]:
    ...
    plt.figure(fign, [10, 11])
    spl = 1
    for myR in [0.5, 0.667, 0.75, 0.833]:
        plt.subplot(2, 2, spl)
        plt.yscale("log")
        plt.xlabel("Eb/No in dB")
        plt.ylabel("Bit Error Rate")
        ...
        indx = np.nonzero(
            np.logical_and(np.logical_and(std == 11, np.abs(R - myR) < 0.01), Z == myZ)
        )[0]
        ...
        indx = np.nonzero(
            np.logical_and(
                np.logical_and(
                    np.logical_and(std == 16, np.abs(R - myR) < 0.01), Z == myZ
                ),
                tt == 0,
            )
        )[0]
        indx = indx[np.argsort(EbNo[indx])]
        if myR == 0.5 or myR == 0.833:
